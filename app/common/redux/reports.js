...
export const fetchGlobalStat = options => invoke({
  ...
  types: ['reports/FETCH_GLOBAL_REQUEST', {
...
export const fetchDetailStat = options => invoke({
  ...
  types: ['reports/FETCH_DETAIL_REQUEST', {
    type: 'reports/FETCH_DETAIL_SUCCESS',
    payload: (action, state, res) => res.json().then(
      json => normalize(json.data, [schemas.detailStat])
    ),
...
export const fetchDeclarationsStat = options => invoke({
  ...
  types: ['reports/FETCH_DECLARATIONS_REQUEST', {
