...
export const fetchMedicalPrograms = options => invoke({
  endpoint: createUrl(`${API_URL}/api/medical_programs`, options),
  method: 'GET',
  headers: {
    'content-type': 'application/json',
  },
  types: ['medical_programs/FETCH_LIST_REQUEST', {
    type: 'medical_programs/FETCH_LIST_SUCCESS',
    payload: (action, state, res) => res.clone().json().then(
      json => normalize(json.data, [medical_program])
    ),
    meta: (action, state, res) => res.clone().json().then(json => json.paging),
  }, 'medical_programs/FETCH_LIST_FAILURE'],
});

export const createMedicalProgram = body => invoke({
  endpoint: createUrl(`${API_URL}/api/medical_programs`),
  method: 'POST',
  headers: {
    'content-type': 'application/json',
  },
  types: ['medical_programs/CREATE_MEDICAL_PROGRAM_REQUEST',
    'medical_programs/CREATE_SUCCESS',
    'medical_programs/CREATE_FAILURE'],
  body,
});

export const deactivateMedicalProgram = id => invoke({
  endpoint: createUrl(`${API_URL}/api/medical_programs/${id}/actions/deactivate`),
  method: 'PATCH',
  headers: {
    'content-type': 'application/json',
  },
  types: ['medical_programs/DEACTIVATE_REQUEST',
    'medical_programs/DEACTIVATE_SUCCESS',
    'medical_programs/DEACTIVATE_FAILURE'],
});

export const fetchMedicalProgram = id => invoke({
  endpoint: createUrl(`${API_URL}/api/medical_programs/${id}`),
  method: 'GET',
  headers: {
    'content-type': 'application/json',
  },
  types: ['medical_programs/FETCH_DETAILS_REQUEST', {
