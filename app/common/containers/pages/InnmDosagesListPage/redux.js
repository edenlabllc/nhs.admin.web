...
export const fetchInnmDosages = options => dispatch =>
  dispatch(fromInnmDosages.fetchInnmDosages(options))
  .then((action) => {
...
const innm_dosages = handleAction(showInnmDosages, (state, action) => action.payload, []);
const paging = handleAction(pagingInnmDosages, (state, action) => action.payload, {});
