...
export const showMedications = createAction('medicationsListPage/SHOW_MEDICATIONS');
export const pagingMedications = createAction('medicationsListPage/ADD_PAGING');

export const fetchMedications = options => dispatch =>
  dispatch(fromMedications.fetchMedications(options))
  .then((action) => {
...
const medications = handleAction(showMedications, (state, action) => action.payload, []);
const paging = handleAction(pagingMedications, (state, action) => action.payload, {});
