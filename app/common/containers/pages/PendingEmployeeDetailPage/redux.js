...
export const fetchEmployee = id => dispatch =>
  ...
  .then((action) => {
    ...
    return dispatch(
      showPendingEmployee(action.payload.entities.employeesRequests[action.payload.result])
    );
  });

const employee = handleAction(showPendingEmployee, (state, action) => action.payload, {});
