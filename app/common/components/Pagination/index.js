...
export default class Pagination extends React.Component {
  ...
  handleLink(page_number, location) {
    return createUrl(this.props.location.pathname, {
      ...location.query,
      page: page_number
    });
  }
  ...
  handlePage(pageIndex) {
    ...
    if (
      ...
      return (
        <li key={page} className={classnames(styles.page, styles.truncated)}>
          ...
        </li>
      );
    }
    ...
    return (
      <div className={styles.pagination}>
        <ul className={styles["g-pagination"]} onClick={this.handleClick}>
          ...
          />
          <ul>{pageArray.map(page => this.handlePage(page))}</ul>
          <PagePrevNext
            buttonType="next"
